...
|----|----------|----------|--------|---------|
| S010 | P2 | Config | implemented | CI workflow guaranteed to fail — ~40 ruff violations block `ruff check src/` |
...
| S015 | P1 | Cross-Reference | implemented | baselines/CRITICAL.md has 4 wrong line references for scoring.py |
| S016 | P1 | Cross-Reference | implemented | evaluation/CRITICAL.md has 5+ stale line references (off by 3-18 lines) |
| S017 | P1 | Cross-Reference | implemented | generation/CRITICAL.md references ranking/scoring.py at wrong lines (192, 202-209) |
| S018 | P1 | Stale Content | implemented | ml/CRITICAL.md says "MPNN data prep script does not exist yet" but it exists |
| S019 | P1 | Stale Content | implemented | GOALS.md Section 4 scoring table shows pre-WS02 "current" methods |
| S020 | P1 | Stale Content | wont-fix | CLAUDE.md says "84 Python source files" and "37 pipeline scripts" — actual is 86 and 40 |
| S021 | P1 | Cross-Reference | implemented | Root CRITICAL.md import list for ranking/scoring.py is incomplete (missing 2 imports) |
...
| S024 | P2 | Cross-Reference | implemented | scripts/generate_vae_candidates.py referenced in INTERFACES.md and CLAUDE.md but does not exist |
...
**Head AI Notes:** All 121 ruff violations fixed 2026-04-06. CI lint passes clean. Violations included F401 unused imports, I001 import sorting, E501 line length, F541 f-strings, and N815 naming across src/.
...
|----|----------|----------|--------|---------|----------------|
| S010 | P2 | Config | implemented | CI workflow guaranteed to fail — ~40 ruff violations block `ruff check src/` | 2026-04-06 |
...
| S015 | P1 | Cross-Reference | implemented | baselines/CRITICAL.md has 4 wrong line references for scoring.py | 2026-04-05 |
| S016 | P1 | Cross-Reference | implemented | evaluation/CRITICAL.md has 5+ stale line references (off by 3-18 lines) | 2026-04-05 |
| S017 | P1 | Cross-Reference | implemented | generation/CRITICAL.md references ranking/scoring.py at wrong lines | 2026-04-05 |
| S018 | P1 | Stale Content | implemented | ml/CRITICAL.md says "MPNN data prep script does not exist yet" but it exists | 2026-04-05 |
| S019 | P1 | Stale Content | implemented | GOALS.md Section 4 scoring table shows pre-WS02 "current" methods | 2026-04-05 |
| S021 | P1 | Cross-Reference | implemented | Root CRITICAL.md import list for ranking/scoring.py is incomplete | 2026-04-05 |
| S024 | P2 | Cross-Reference | implemented | scripts/generate_vae_candidates.py referenced but does not exist | 2026-04-05 |
...
**S010** (implemented): All 121 ruff violations fixed 2026-04-06. Violations spanned F401 unused imports, I001 import sorting, E501 line length, F541 f-strings, and N815 naming across src/. CI lint now passes clean.
...
**S015** (implemented): Updated 4 stale line references in baselines/CRITICAL.md: docking stub 135-149→202-216, _score_druglikeness 83-129→101-147, baseline weights 176-180→269-273, unified weights ranking/scoring.py:40-45→86-91. Added function name anchors.

**S016** (implemented): Updated 5 stale line references in evaluation/CRITICAL.md: run_full_comparison 176-179→179-183, ComparativeResult 75-86→75-89, sub-result types 24-72→25-72, compute_novelty 152-173→155-176. Updated run_full_comparison signature to show run_statistics parameter.

**S017** (implemented): Fixed 2 cross-module references in generation/CRITICAL.md: rank_state_aware line 192→288, deduplication 202-209→303-306.

**S018** (implemented): Rewrote first bullet in ml/CRITICAL.md. Now reflects: VAE trained (8,109 SMILES, epoch 29), MPNN/ADMET in GPU queue, all 3 data prep scripts exist. Also fixed vocab filename vocab.json→vocabulary.json.

**S019** (implemented): Updated GOALS.md Section 4 scoring table "Method (current)" column to show post-WS02 reality: Morgan/ECFP4 Tanimoto, RDKit QED+Lipinski+SA, 3-tier cascade. Added Status column. Originally marked wont-fix but addressed during documentation sweep.

**S021** (implemented): Updated CRITICAL.md line 40 import list to include all 6 imports (_has_rdkit, _score_druglikeness, _score_druglikeness_enhanced, _score_reference_similarity, _score_docking_stub, _tanimoto_ngram) and corrected line range 19-24→19-26.

**S024** (implemented): Updated docs/ai-guide/ml-models.md to note that scripts/generate_vae_candidates.py does not exist yet and needs to be created to sample from the trained VAE.
